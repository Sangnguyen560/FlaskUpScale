...
class MLP_SR(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))
        x = torch.sigmoid(self.fc4(x))  # Output trong khoảng [0,1]
        x = x.view(-1, 1, 64, 64)  # Reshape thành ảnh
        return x

# Load model đã train
model = MLP_SR().to(device)
model.load_state_dict(torch.load("mlp_sr_model.pth", map_location=device))
model.eval()
...
def upload_file():
    if request.method == 'POST':
        ...
        if file:
            ...
            <html>
            <head>
                <title>MLP Image Upscaling</title>
                <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
            </head>
            <body class="container py-5">
                <h1 class="text-center mb-4">MLP Uploaded Image and Upscaled Version</h1>
